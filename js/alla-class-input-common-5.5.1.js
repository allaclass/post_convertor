...
function info_input() {
  ...
  // 내용만 발라내기
  let infoDataObject = infoData.substring(infoObject + 5, infoKind).replace(/(^\s*)|(\s*$)/gi, ''); // 과목명
  let infoDataKind = infoData.substring(infoKind + 6, infoYear).replace(/(^\s*)|(\s*$)/gi, ''); // 종류
  let infoDataYear = infoData.substring(infoYear + 6, infoSeme).replace(/(^\s*)|(\s*$)/gi, ''); // 연도
  let infoDataSeme = infoData.substring(infoSeme + 6, infoGrade).replace(/(^\s*)|(\s*$)/gi, ''); // 학기
  let infoDataGrade = infoData.substring(infoGrade + 6, infoProf).replace(/(^\s*)|(\s*$)/gi, ''); // 학년
  let infoDataProf = infoData.substring(infoProf + 5, infoRange).replace(/(^\s*)|(\s*$)/gi, ''); // 교수명
  let infoDataRange = infoData.substring(infoRange + 5, infoSNum).replace(/(^\s*)|(\s*$)/gi, ''); // 셤범위
  let infoSNumData = infoData.substring(infoSNum + 5, infoENum).replace(/(^\s*)|(\s*$)/gi, ''); // 문제 시작 번호
  let infoENumData = infoData.substring(infoENum + 5, infoThumb).replace(/(^\s*)|(\s*$)/gi, ''); // 문제 종료 번호
  let infoDataThumb = infoData.substring(infoThumb + 5, infoAnswer).replace(/(^\s*)|(\s*$)/gi, ''); // 썸네일
  let infoDataAnswer = infoData.substring(infoAnswer + 6, infoLength).replace(/(^\s*)|(\s*$)/gi, ''); // 정답
