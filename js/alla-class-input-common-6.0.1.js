...
// 1. 텍스트박스 주소값 지정
let txtBoxContent = document.getElementById('contents');
let txtBoxResult = document.getElementById('result');
let txtBoxFinal = document.getElementById('final');
...
let fnQuestionInfo = () => {
  ...
  // 1-3. 내용만 발라내서 변수에 할당
  // 1-3-1. 과목명
  infoDataObject = infoDataAll
    .substring(infoIndexObject + 5, infoIndexKind)
    .replace(/(^\s*)|(\s*$)/gi, '');
  // 1-3-2. 종류
  infoDataKind = infoDataAll
    .substring(infoIndexKind + 6, infoIndexYear)
    .replace(/(^\s*)|(\s*$)/gi, '');
  // 1-3-3. 연도
  infoDataYear = infoDataAll
    .substring(infoIndexYear + 6, infoIndexSeme)
    .replace(/(^\s*)|(\s*$)/gi, '');
  // 1-3-4. 학기
  infoDataSeme = infoDataAll
    .substring(infoIndexSeme + 6, infoIndexGrade)
    .replace(/(^\s*)|(\s*$)/gi, '');
  // 1-3-5. 학년
  infoDataGrade = infoDataAll
    .substring(infoIndexGrade + 6, infoIndexProf)
    .replace(/(^\s*)|(\s*$)/gi, '');
  // 1-3-6. 교수명
  infoDataProf = infoDataAll
    .substring(infoIndexProf + 5, infoIndexRange)
    .replace(/(^\s*)|(\s*$)/gi, '');
  // 1-3-7. 셤범위
  infoDataRange = infoDataAll
    .substring(infoIndexRange + 5, infoIndexStartNum)
    .replace(/(^\s*)|(\s*$)/gi, '');
  // 1-3-8. 문제 시작 번호
  infoDataStartNum = Number(
    infoDataAll
      .substring(infoIndexStartNum + 5, infoIndexEndNum)
      .replace(/(^\s*)|(\s*$)/gi, '')
  );
  // 1-3-9. 문제 종료 번호
  infoDataEndNum = Number(
    infoDataAll
      .substring(infoIndexEndNum + 5, infoIndexThumb)
      .replace(/(^\s*)|(\s*$)/gi, '')
  );
  // 1-3-10. 썸네일
  infoDataCountNum = infoDataEndNum - infoDataStartNum + 1;
  infoDataThumb = infoDataAll
    .substring(infoIndexThumb + 5, infoIndexAnswer)
    .replace(/(^\s*)|(\s*$)/gi, '');
  // 1-3-11. 정답
  infoDataAnswer = infoDataAll
    .substring(infoIndexAnswer + 6, infoIndexLength)
    .replace(/(^\s*)|(\s*$)/gi, '');
};
